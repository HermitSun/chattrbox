...
const chatRooms = {}; // [roomName: string]: Message[]
...
ws.on('connection', function (socket, request) {
  ...
  socket.on('message', function (data) {
    ...
    if (JSON.parse(data).targetChatRoom) { // 这种消息是专门用来建立聊天室的，不应该push进消息数组
      ...
      chatRooms[currentChatRoom] = chatRooms[currentChatRoom] || []; // 如果没有就新建一个
      chatRooms[currentChatRoom].forEach((msg) => {
        socket.send(msg); // 建立连接后把当前聊天室的消息发过去
      });
      socket.send(JSON.stringify({
        chatRooms: Object.keys(chatRooms), // 顺便告诉他还有哪些可选的聊天室
        currentChatRoom                    // 以及现在是哪个聊天室（类似于握手的实现，双重确认）
      }));
    } else { // 正常的消息就正常添加和发送
      chatRooms[currentChatRoom].push(data);
